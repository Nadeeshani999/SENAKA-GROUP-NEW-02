...
async function initFillingPage() {
  ...
  /* ── Table renderer — columns match HTML thead exactly ─────────── */
  rows._renderer = r => {
    const pL   = r[COL.petrolL]||0;
    const dL   = r[COL.dieselL]||0;
    const pLKR = r[COL.petrolLKR]||0;
    const dLKR = r[COL.dieselLKR]||0;
    const totVL= pL + dL + (r[COL.euro3L]||0) + (r[COL.extraMileL]||0);
    const totRL= pLKR + dLKR + (r[COL.euro3LKR]||0) + (r[COL.extraMileLKR]||0);
    return `<tr>
      <td>${fmtDate(r['Date'])}</td>
      <td class="num">${pL.toFixed(0)}</td>
      <td class="num">${dL.toFixed(0)}</td>
      <td class="num">Rs. ${fmtLKR(pLKR,true)}</td>
      <td class="num">Rs. ${fmtLKR(dLKR,true)}</td>
      <td class="num">${totVL.toFixed(0)}</td>
      <td class="num text-green">Rs. ${fmtLKR(totRL,true)}</td>
    </tr>`;
  };
